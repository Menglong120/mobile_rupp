...
class ProductApiController extends Controller
{
     ...
     */
    protected function removeBackground($imagePath): ?string
    {
        ...
        try {
            ...
            if ($response->getStatusCode() === 200) {
                $newImageName = 'nobg_' . basename($imagePath);
                $newPath = 'products/' . $newImageName;
                Storage::disk('public')->put($newPath, $response->getBody());
            ...
            \Illuminate\Support\Facades\Log::error('Remove.bg error: ' . $e->getMessage());
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                // Try removing background if API key is set
                $nobgPath = $this->removeBackground($path);
                $product->image = $nobgPath ?? $path;
            }
    ...
    protected function formatProductResponse($product)
    {
        $image = $product->image;
        if ($image) {
            // Build image URL from incoming request host so Web and emulator both work.
            if (!str_starts_with($image, 'http')) {
                $root = request()->getSchemeAndHttpHost();
                $image = $root . "/api/products/{$product->pid}/image";
            }
        }
