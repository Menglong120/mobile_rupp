...
class OrderApiController extends Controller
{
    ...
    public function checkPaymentStatus(Request $request, BakongService $bakongService, TelegramService $telegramService)
    {
        ...
        // ── Fast path #1: PaymentRequest marked paid (if ID is UUID)
        if (Str::isUuid($id)) {
            $paymentRequest = PaymentRequest::find($id);
            if ($paymentRequest && ($paymentRequest->status === 'paid' || $paymentRequest->status === 'completed')) {
        ...
        if ($isMock) {
            ...
            // Ensure ID is passed as string to avoid any further numeric conversion internal to the service
            $status = $bakongService->checkTransactionStatus((string)$id);
        }
        ...
        if ($status['status'] === 'SUCCESS') {
            if ($paymentRequest) {
                ...
                $paymentRequest->save();
                $this->finalizePaymentRequest($paymentRequest, $telegramService);
            } elseif ($existingOrder && $existingOrder->status !== 'paid' && $existingOrder->status !== 'completed') {
    ...
    public function handleBakongCallback(Request $request, BakongService $bakongService, TelegramService $telegramService)
    {
        ...
        if ($paymentRequest) {
            ...
            if ($finalOrder && $finalOrder->status !== 'paid' && $finalOrder->status !== 'completed') {
    ...
    /**
    * Demo endpoint: Mark a payment request as paid and finalize it
     * For testing KHQR without actual scanning
     */
    public function markPaymentCompleted(Request $request, TelegramService $telegramService)
    {
        $validator = Validator::make($request->all(), [
        ...
        $id = $request->payment_request_id;

        // Only works for PaymentRequests (UUID)
        if (!Str::isUuid($id)) {
            return response()->json(['message' => 'Invalid payment request ID'], 400);
        }
        ...
        $existingOrder = Order::where('payment_id', $id)->first();
        if ($existingOrder) {
            return response()->json([
                'success' => true,
                'message' => 'Payment already finalized',
                'order_id' => $existingOrder->oid,
            ], 200);
        }
        ...
        $order = $this->finalizePaymentRequest($paymentRequest, $telegramService);

        return response()->json([
            'success' => true,
            'message' => 'Payment marked as paid and order created',
            'order_id' => $order->oid,
        ], 200);
    }
}