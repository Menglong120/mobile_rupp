...
class BakongService
{
    ...
    public function generateKhqr($order, $currency = 'USD')
    {
        ...
        $tag62_08   = $this->formatTag('08', 'Payment for Order #' . $orderId);
        $payload   .= $this->formatTag('62', $tag62_01 . $tag62_08);

        // Tag 99: Custom data or Expiration (Some banks require Tag 99 for specific data)
        // If your bank app is strict, we should remove Tag 99 or format it exactly as per their spec.
        // Let's try removing Tag 99 if it causes "Invalid" on real bank apps, or keep it if it's standard.
